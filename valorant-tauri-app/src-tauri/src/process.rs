...
// S3: 白名单——只允许操作这几个进程，其余一律拒绝
pub(crate) const ALLOWED_PROCESSES: &[&str] = &[
    "SGuard64.exe",
    "SGuardSvc64.exe",
    "SGuardUpdate64.exe",
];

pub(crate) fn is_allowed_process(process_name: &str) -> bool {
    ALLOWED_PROCESSES
        .iter()
...
pub(crate) unsafe fn enable_debug_privilege() -> Result<(), String> {
...
// 修改某个进程的 CPU 亲和性和优先级
#[tauri::command]
pub fn fix_process_cpu_and_affinity(process_name: &str) -> (bool, String) {
    // S3: 白名单检查——只允许操作指定进程
...
// E2: 恢复进程的原始亲和性和优先级
#[tauri::command]
pub fn restore_process(process_name: &str) -> (bool, String) {
    // 从存储中取出原始状态（同时移除条目）
    let state = {
        ...
        match states.remove(process_name) {
            ...
            None => {
                info!(
                    "未找到进程 {} 的保存状态，可能未曾优化或已恢复",
                    process_name
                );
                return (
                    true,
                    format!("{} 无需恢复（未找到保存的原始状态）", process_name),
                );
            }
        }
    };
    ...
    unsafe {
        ...
        match OpenProcess(
            ...
            Ok(handle) => {
                ...
                if let Err(e) =
                    SetPriorityClass(handle, PROCESS_CREATION_FLAGS(state.priority_class))
                {
                    let _ = CloseHandle(handle);
                    return (
                        false,
